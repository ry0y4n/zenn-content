...
- **コピペ元が散らばっている**: 社内の Agent Skill 置き場リポ、Anthropic の [skills](https://github.com/anthropics/skills)、[GitHub Awesome Copilot](https://github.com/github/awesome-copilot) …等、あちこちから手で拾ってくる
- **コピペ先が増え続ける**: 拾ってきたものを Copilot / Claude / Cursor / OpenCode それぞれの置き場にコピーして回る
  :::

絵にすると、こんな **「多対多のコピペ配管」** が各開発者の手元で発生している状態です。

```mermaid
flowchart LR
    subgraph SRC ["🌀 コピペ元（散在）"]
    ...
    dev("👩‍💻 開発者<br/>手でコピペ & バージョン管理")

    subgraph DST ["📦 コピペ先（ハーネスごとに線形増加）"]
...
- **コピペ元の更新に追従できない**: 外部 OSS のスキル集がアップデートされても気付かない／拾い直しが手作業
- **「社内リポにコピーして溜める」運用は二重保持**: 元が進化しても社内コピーは古いまま、どちらが正か分からなくなる
- **ガイドに「ここからコピーして」と書く運用はヒューマンエラーの温床**: 誰がどのバージョンを拾ったかが残らず、バージョン指定もしづらい
- **コピペ先が増え続ける**: ハーネスごとに置き場が違うので、同じ内容を複数箇所にコピー
- **新メンバーが入るたびにセットアップ作業**: 「このリポのここと、あのリポのここをコピーして…」
...
| 従来                                               | APM                                                   |
| -------------------------------------------------- | ----------------------------------------------------- |
| Slack で「ここから最新版コピペしてください」と告知 | `apm.yml` の ref を `v1.1` にバンプして PR            |
| メンバーごとに反映タイミングがバラバラ             | **マージ後、全員が `git pull && apm install` で揃う** |
| 反映漏れを検知できない                             | `apm.lock.yaml` の diff がレビューに現れる            |
