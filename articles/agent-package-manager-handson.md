...
| **③ `apm-policy.yml`**                    | 会社ルールを **ローカル `apm install` と CI `apm audit` の両方で強制**（ハンズオン②で実物を見せます） |
...
③ の `apm-policy.yml` については、配置場所や書き方・ローカル/CI それぞれでの効き方などまとまった説明が必要なので、これはハンズオン② の冒頭でまとめて扱います。
